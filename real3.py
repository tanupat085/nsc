...
Builder.load_file(dir_path + '/real3.kv')

Pause_1 = False



class Checkup_3(Widget):
...
class Level_3(Widget) :
    pass

class Background_Bonus_3(Widget):
    pass


class Pushup_3(Widget):
    upup = bool
...
class Gamemain_3(Widget):
    ...
    sound1 = SoundLoader.load("musiccorrect.wav")
    sound2 = SoundLoader.load("musicwrong.wav")
...
class Itemgame_3(Widget):
    image = ObjectProperty(None)
    velocity_x = NumericProperty(0)
    velocity_y = NumericProperty(0)
    velocity = ReferenceListProperty(velocity_x, velocity_y)

    picturelist = ["news","can","waterbottle","bottle","bucket","battery","ink","plasticbag","phone","brokenbottle"]
    ...
    def setpic(self):
        ...
        if self.picture == "waterbottle" :
            ...
            self.itemtype = 0
        if self.picture == "bottle":
            self.image.source = "item/bottlegood.png"
            self.itemtype = 0
        if self.picture == "bucket":
            self.image.source = "item/bucketgood.png"
            self.itemtype = 0
        if self.picture == "ink":
            self.image.source = "item/iinkbad.png"
            self.itemtype = 1
        if self.picture == "plasticbag" :
            self.image.source = "item/plasticbad.png"
            self.itemtype = 1
        ...
        if self.picture == "brokenbottle" :
            ...
            self.itemtype = 1
        if self.picture == "battery" :
            self.image.source = "item/batterybad.png"
            self.itemtype = 1
...
class Pausebutt_3(Button):
    pause = False

    def on_press(self):
        self.pause = not self.pause
        
class Resumebutt_3(Button):
    resume = True
    def on_press(self):
        self.resume = not self.resume

class Clickbutt_3(Button):
    move_up = True
    tell = True
