...
Builder.load_file(dir_path + '/real2.kv')
...
class Level_2(Widget) :
    pass

class Background_Bonus_2(Widget):
    pass


class Pushup_2(Widget):
    upup = bool
...
class Itemgame_2(Widget):
    image = ObjectProperty(None)
    velocity_x = NumericProperty(0)
    velocity_y = NumericProperty(0)
    velocity = ReferenceListProperty(velocity_x, velocity_y)

    picturelist = ["news","can","waterbottle","bottle","ink","plasticbag","phone","brokenbottle"]
...
class Pausebutt_2(Button):
    pause = False

    def on_press(self):
        self.pause = not self.pause
        
class Resumebutt_2(Button):
    resume = True
    def on_press(self):
        self.resume = not self.resume

class Clickbutt_2(Button):
    move_up = True
    tell = True
